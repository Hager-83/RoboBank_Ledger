...
int out_total_deposits = 0, out_total_withdrawals=0, out_total_fees = 0, out_total_interest =0, out_net_exposure = 0 ;
    ...
    else
    {
        ...
        switch(tx_type)
        {
            ...
            case 4: /** TransferIn **/
                ...
                out_total_deposits += amount_cents;
            break;

            case 1: /** Withdrawal **/
            case 5: /** TransferOut **/
                balance = apply_withdrawal(balance, amount_cents);
                out_total_withdrawals += amount_cents;
            break;

            case 2: /** Fee **/
                balance = apply_fee(balance, (int)amount_cents);
                out_total_fees += amount_cents;
            break;
            ...
            case 3: /** Interest --> "Simple Intreset"  **/
            {
                ...
                out_total_interest += interest_amount;
            break;
            }
...
int balance_of(string ac_account_id[],int ac_balance[],int ac_count,string account_id)
{
    ...
    {   /* Account Found, return Balance */
        ret_val = ac_balance[ret_val];
        if(!ret_val)
        {
            cout << "Account Balance = "<< ret_val << endl;
        }
        else{/*Nothing*/}
    }
    else
...
/*************** 6.Bank Summary **************/
void bank_summary(int ac_count, int ac_balance[], int *out_total_deposits, int *out_total_withdrawals,int * out_total_fees,int * out_total_interest,int& out_net_exposure )
 {
    for(auto i =0; i<ac_count ; i++)
    {
        out_net_exposure += ac_balance[i];
    }
    cout << "out total deposits    = " <<out_total_deposits << "Cents" << endl;  
    cout << "out total withdrawals = " <<out_total_withdrawals << "Cents" <<endl;  
    cout << "out total fees        = " <<out_total_fees << "Cents" <<endl;  
    cout << "out total interest    = " <<out_total_interest << "Cents" <<endl;  
    cout << "out net exposure      = " <<out_net_exposure<< "Cents" <<endl;
 }
