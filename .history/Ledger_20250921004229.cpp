...
/***************** 3. Apply One *****************/
void apply_one(string ac_account_id[], int ac_balance[], int ac_capacity, int& ac_count, string account_id,int tx_type,int amount_cents)
{
    int index   = -1;
    int balance = 0;
    ...
    else
    {
        ...
        switch(tx_type)
        {
            ...
            case 2: /** Fee **/
                balance = apply_fee(balance, amount_cents);
            break;

            case 3: /** Interest **/
                int  days, day_count_basis;
                float apr;
                char rounding_policy;
                    cout << "Enter annual rate ";
                    cin >> apr;
                    cout << "Enter days to accure ";                        cin >> days;
                            ...
                            interest_amount = simple_interest(base,apr, days, day_count_basis, rounding_policy);

                            cout << "---------------------------------\n";
                            cout << "Simple Interest Result: " << interest_amount << " cents\n";
                            cout << "New Balance (Principal + Interest): " << base + interest_amount << " cents\n";
                            cout << "---------------------------------\n";

                balance = simple_interest(balance, apr, days, day_count_basis, rounding_policy);
            break;


        }
        cout << "---------------------------------\n";
        cout << "Operation done."<<endl<< "New balance = " << balance <<" cents." << endl;
        cout << "---------------------------------\n";
